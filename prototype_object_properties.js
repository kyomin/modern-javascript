...
// [1]: constructor 속성
// 이 속성은 함수(예: Animal)를 가리킨다. --> 참조 --> 즉, 참조를 값으로 가지는 속성이다.

// [2]: 서로가 참조 --> 서로가 연결고리 역할
...
a1.__proto__ = Animal2.prototype; // 프로토타입 체인으로 참조하는 부모 객체를 Animal2의 프로토타입 객체로 바꿈
a1.bbb(); // bbb <- a1.aaa()를 호출하면 당연히 에러!
console.log('----------------------------------------');

// [3]: new 연산자에 의해 새롭게 생성된 객체 --> 인스턴스
// 생성자 함수(Animal) <--> new 연산자로 생성된 객체(tiger, lion)
// 이 둘의 관계는 딱히 없지만 중요한게 있다면 --> new 연산자가 생성자 함수를 이용해서 새로운 객체를 생성.
// 하지만, 이렇게 생성된 객체들(tiger, lion)의 원형(부모)은 '(Animal) 프로토타입 객체'
// 프로토타입 체인이 어떻게 연결되는지를 잘 기억하는게 중요!

// [4]: 인스턴스(new로 생성된 객체)가 어떤 생성자 함수로 생성된 것인지를 알아내는 방법이 있을까?
...
console.log(A.prototype.__proto__); // 무엇을 가리키는가? --> 최상위 Object 프로토타입 객체(Object.prototype)
...
console.log('----------------------------------------');
console.log(a1 instanceof Animal); // false --> __proto__를 Animal2의 원형으로 바꿨으므로
console.log(a1 instanceof Animal2); // true

// 정리
// instanceof 연산자는 특정 객체가 그 객체와 연결된,
// 즉, 프로토타입 체인에 연결(포함)되어 있는지를 체크하는 방법중 하나이다.
